...
public class SysDictTypeServiceImpl extends ServiceImpl<SysDictTypeMapper, SysDictType> implements ISysDictTypeService {
    ...
    @Override
    public Map<String, List<SysDictData>> selectTypeList(List<String> dictTypeList) {
        Map<String, List<SysDictData>> map = new HashMap<>(16);
        List<SysDictData> list = null;
        for (String type : dictTypeList) {
            if (null != (list = this.selectDictDataByType(type))){
                map.put(type,list);
            }
        }
        return map;
