<template>
	<span class="tree-expand">
		<span class="tree-label" v-show="DATA.isEdit">
			...
			v-model="DATA.categroyName"
			:ref="'treeInput'+DATA.categroyId"
			@click.stop.native="nodeEditFocus"
			@blur.stop="nodeEditPass(STORE,DATA,NODE)"
			@keyup.enter.stop.native="nodeEditPass(STORE,DATA,NODE)"></el-input>
		</span>
		<span v-show="!DATA.isEdit" 
		:class="[DATA.categroyId > maxexpandId ? 'tree-label' : 'tree-label']">
			<span>{{DATA.categroyName}}</span>
		</span>
	...
	export default{
		...
		methods: {
			...
			nodeEdit(s,d,n){//编辑
				d.isEdit = true;
				this.$nextTick(() => {
					this.$refs['treeInput'+d.categroyId].$refs.input.focus()
				})
