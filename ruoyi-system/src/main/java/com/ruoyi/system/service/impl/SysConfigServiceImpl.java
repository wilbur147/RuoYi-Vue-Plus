...
public class SysConfigServiceImpl extends ServiceImpl<SysConfigMapper, SysConfig> implements ISysConfigService {
    ...
    public void saveConfig(String key, String value) {
        if (!StrUtil.isEmpty(key)) {
            String PrefixKey = SystemConfigKeyEnum.SYS_CONFIG_PREFIX.getKey().concat(key);
            SysConfig config = this.getSysConfigByKey(PrefixKey);
            if (ObjectUtil.isEmpty(config)) {
                config = new SysConfig();
                config.setConfigName(SystemConfigKeyEnum.getValueByKey(key));
                config.setConfigKey(PrefixKey);
    ...
    private SysConfig getSysConfigByKey(String key) {
        if (StringUtils.isEmpty(key)) {
            return null;
        }
        return baseMapper.selectOne(new LambdaQueryWrapper<SysConfig>().eq(SysConfig::getConfigKey, key));
    }


    @Override
    public Map<String, String> getAll(String configKeywords) {
        List<SysConfig> sysConfigs = baseMapper.selectList(new LambdaQueryWrapper<SysConfig>().like(SysConfig::getConfigKey, configKeywords));
        // 抽取对象中的key和value
        Map<String,String> normalMap = new HashMap<>(16);
        for (SysConfig sysConfig : sysConfigs) {
            String configKey = sysConfig.getConfigKey();
            normalMap.put(configKey.substring(configKey.lastIndexOf(".")+1),sysConfig.getConfigValue());
        }
        return normalMap;
    }
